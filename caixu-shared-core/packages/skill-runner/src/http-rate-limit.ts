...
type RetryableTransportError = Error & {
  code?: string;
  cause?: unknown;
};

type FetchWithRateLimitRetryInput = {
  scope: string;
  url: string;
  init: RequestInit;
  timeoutMs?: number;
...
export type HttpRateLimitEvent = {
  source: "skill-runner-http";
...
function isRetryableTransportError(error: unknown): error is RetryableTransportError {
  ...
  return (
    typeof (error as RetryableTransportError).code === "string" ||
...
export async function fetchWithRateLimitRetry(
  input: FetchWithRateLimitRetryInput
): Promise<Response> {
    ...
    const result = await withScopeThrottle(input.scope, async (state) => {
      ...
      try {
        response = await fetch(input.url, {
          ...input.init,
          signal:
            typeof AbortSignal !== "undefined" && input.timeoutMs
